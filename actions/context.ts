...
import { generateNthPhasePrompt, generatePRDPrompt, generateProjectStructurePrompt, generateUIUXPrompt, numberOfPhasesPrompt } from "../prompts/Docs";
...
export async function numberOfPhases(conversationHistory: any[] = [], architectureData: any) {
    ...
    const template = numberOfPhasesPrompt;
...
export async function generatePRDStreaming(
  ...
  const template = generatePRDPrompt;
  const prompt = PromptTemplate.fromTemplate(template);
  const formattedHistory = conversationHistory.map(msg => 
    `${msg.type === 'user' ? 'User' : 'Assistant'}: ${msg.content}`
  ).join('\n');
...
export async function generateProjectStructureStreaming(
  ...
  const template = generateProjectStructurePrompt
  const prompt = PromptTemplate.fromTemplate(template);
...
export async function generateUIUXStreaming(
  ...
  const template = generateUIUXPrompt
  const prompt = PromptTemplate.fromTemplate(template);
...
export async function generateNthPhaseStreaming(
  ...
  const template = generateNthPhasePrompt;
  const prompt = PromptTemplate.fromTemplate(template);
